...
class DifferentiableSTL(torch.nn.Module, metaclass=ABCMeta):
    '''
    ...
    def _robustness_trace(
        ...
        """This is the function which assumes inputs in the env are time
        reversed."""

        raise NotImplementedError("robustness_trace not yet implemented")

    def robustness_trace(self, env, time=0, pscale=1, scale=-1, keepdim=True, agm=False,distributed=False, **kwargs):
        '''
        ...
        '''
        reversed_input_env = self._reverse_env_inputs(env)
        reversed_trace = self.forward(reversed_input_env, pscale=pscale, scale=scale, keepdim=keepdim, agm=agm, distributed=distributed, **kwargs)
        trace = torch.flip(reversed_trace, [1])
        return trace

    def robustness(self, env, time=0, pscale=1, scale=-1, keepdim=True, agm=False,distributed=False, **kwargs):
        '''
        Extracts the robustness_trace value at the given time.
        Default: time=0 assuming this is the index for the NON-REVERSED trace. But the code will take it from the end since the input signal is TIME REVERSED.
        ...
        reversed_input_env = self._reverse_env_inputs(env)
        reversed_trace = self.forward(reversed_input_env, pscale=pscale, scale=scale, keepdim=keepdim, agm=agm, distributed=distributed, **kwargs)
        return reversed_trace[:, -(time+1)].unsqueeze(1) # [batch_size, 1]

    def eval_trace(self, env, pscale=1, scale=-1, keepdim=True, agm=False, distributed=False, **kwargs):
        '''
    ...
    def eval(self, env, time=0, pscale=1, scale=-1, keepdim=True, agm=False, distributed=False, **kwargs):
        '''
        ...
        reversed_input_env = self._reverse_env_inputs(env)
        reversed_trace = self.forward(reversed_input_env, pscale=pscale, scale=scale, keepdim=keepdim, agm=agm, distributed=distributed, **kwargs)
        return reversed_trace[:,-(time+1)].unsqueeze(1) > 0 # [batch_size, 1]
...
class TemporalOperator(DifferentiableSTL):
    '''
    ...
    def __init__(
        ...
        # rnn_dim = 1 if the interval is None or [0, ∞) otherwise rnn_dim=end of interval
        ...
        shift_mat = torch.tensor(np.diag(np.ones(self.rnn_dim-1), k=1)).requires_grad_(False).float()
        self.register_buffer("shift_mat", shift_mat)

        append_vec = torch.zeros(self.rnn_dim).requires_grad_(False).float()
        append_vec[-1] = 1.0
        self.register_buffer("append_vec", append_vec)
    ...
    def _run_cell(self, x, scale, agm=False, distributed=False):
        ...
        outputs = []
        states = []
        hc = self._initialize_rnn_cell(x)                                # [batch_size, rnn_dim]
    ...
    def _robustness_trace(self, inputs, pscale=1, scale=-1, keepdim=True, agm=False, distributed=False, **kwargs):
        ...
        # to the temporal operator graph.
